...
NTSTATUS h_NtQuerySystemInformation(uint32_t SystemInformationClass, uintptr_t SystemInformation, ULONG SystemInformationLength, PULONG ReturnLength)
{
    ...
    if (x == 0){
        ...
        else if (SystemInformationClass == 0x4D) { //SystemModuleInformationEx
            auto ptr = (char*)SystemInformation;
            //*(uint64_t*)(ptr + 0x18) = GetModuleBase("ntoskrnl.exe");
        ...
        else if (SystemInformationClass == 0x5a) { //SystemBootEnvironmentInformation
        ...
        else if (SystemInformationClass == 0x4C) { //SystemFirmwareTableInformation
            
        }
        else if (SystemInformationClass == 0x67) { //SystemCodeIntegrityInformation
            
        }
        else if (SystemInformationClass == 0x5) { //SystemProcessInformation

        }
    }
    else 
    {
        Logger::Log("\tClass %08x Failed\n", SystemInformationClass);
    }
...
NTSTATUS h_RtlInitUnicodeString(PUNICODE_STRING DestinationString, PCWSTR SourceString) {
    ...
    Logger::Log("\tRtlInitUnicodeString : %ls = %llx\n", SourceString, ret);
    return ret;
}
...
NTSTATUS h_PsSetCreateProcessNotifyRoutineEx(void* NotifyRoutine, BOOLEAN Remove) {
    if (Remove) {
        ...
        Logger::Log("\tCreateProcessNotifyRoutine addr=%p\n", NotifyRoutine);
        return STATUS_SUCCESS;
    }
}

NTSTATUS h_PsSetCreateThreadNotifyRoutine(PVOID NotifyRoutine) {
    Logger::Log("\tThreadNotifyRoutine addr=%p\n", NotifyRoutine);
    return STATUS_SUCCESS;
}

NTSTATUS h_PsSetLoadImageNotifyRoutine(PVOID NotifyRoutine) {
    Logger::Log("\tLoadImageNotifyRoutine addr=%p\n", NotifyRoutine);
    return STATUS_SUCCESS;
}
...
unsigned long long h_MmGetPhysicalAddress(uint64_t BaseAddress) { //To test shit
    ...
    Logger::Log("\tInput address 0x%llx\n", BaseAddress);
...
VOID h_MmProbeAndLockPages(_Inout_ PMDL MemoryDescriptorList, _In_ /*KPROCESSOR_MODE*/ int AccessMode, _In_ LOCK_OPERATION Operation) { }

typedef enum _MEMORY_CACHING_TYPE_ORIG
{ MmFrameBufferCached = 2 } MEMORY_CACHING_TYPE_ORIG;
...
PVOID h_MmMapLockedPagesSpecifyCache(_Inout_ PMDL MemoryDescriptorList,
