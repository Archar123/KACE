   ...
   // exceptionMutex.lock();
    ...
    if (e->ExceptionRecord->ExceptionCode == EXCEPTION_FLT_DIVIDE_BY_ZERO) {
        ...
        case WRITE_VIOLATION:
            ...
            // exceptionMutex.unlock();
            Logger::Log("[E]MemoryWrite Not Handle\n");
            exit(0);
            break;
...
DWORD FakeDriverEntry(LPVOID) {
    ...
    MemoryTracker::TrackVariable((uintptr_t)drvObj.DriverSection, sizeof(UINT64), "MainModule.DriverObject.DriverSection.KLdr");
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1)
        ...
        DriverPath = "d:\\kace\\emu\\easyanticheat_2.sys";
